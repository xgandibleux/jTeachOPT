...
# jTeachOPT
#
#  Heuristiques et metaheuristiques illustrees sur le 01UKP sous JULIA
#  Algorithmes exacts et approches dedies au 01UKP sous JULIA
#
#  Xavier Gandibleux (Xavier.Gandibleux@univ-nantes.fr)
#  Universite de Nantes, Faculte des sciences et techniques - IRCCyN UMR CNRS 6597
#
#  Ver 0.1.1 - Released : 17 Mars 2016
...
# Algorithmes/traitements implementes :
#
#  solution gloutonne pour 01UKP
#  solution aleatoire realisable pour 01UKP
#  relaxation lineaire du 01UKP
#  heuristique d'exploration (heuExplore)
#  metaheuristique recuit simule (metaSA)
#  mouvement swap pour 01UKP (swap)
#  mouvement add_or_drop pour 01UKP (addOrDrop)
#  calcul et tracage de schema de refroidissement (examenRefroidissement)

# ============================================================

# ------------------------------------------------------------
# constantes globales (besoin de tracage de l'algo)
...
# ------------------------------------------------------------
# simple exploration du voisinage d'une solution a partir d'un swap aleatoire
...
# ------------------------------------------------------------
# creation aleatoire d'une instance de sac-a-dos et de solutions realisables

n = 100  # taille de l'instance
max_ci = 20 # c_i \in [1,max_ci]
max_wi = 30 # w_i \in [1,max_wi]

# --- creation de l'instance
# instance un peu particuliere de facon a se faciliter la vie pour le calcul du glouton
ukp   = instance(n, sort(rand(1:max_ci,n), rev=true), sort(rand(1:max_wi,n)), 0)
ukp.W = round(Int64, sum(ukp.w)/2)

# ---
# Calcule la solution gloutonne avec pour utilite : u(i) = c(i)/p(i)
sGreedy = solution(zeros(Int64, n), [], [], 0, 0)
sommeW = 0
for i = 1:n
  #@printf "%f\n" ukp.c[i]/ukp.w[i]
...
sGreedy.z = fct(ukp.c, sGreedy.x)
sGreedy.r = ukp.W - dot(ukp.w, sGreedy.x)
somX=sum(sGreedy.x) # somme des x_i = 1

# ---
# calcule la relaxation lineaire
s = indexin([1], sGreedy.x) # identifie la derniere variable a 1 dans x
if (ukp.W - dot(ukp.w, sGreedy.x)) > 0
  # contrainte non saturee => ajout de la partie fractionnaire de l'item bloquant
  cw = ukp.c[s+1]/ukp.w[s+1] #astuce
  zRelax = sGreedy.z + (ukp.W - dot(ukp.w, sGreedy.x)) * cw[1]
else
...
s0 = solution(zeros(Int64, n), [], [], 0, 0)
semence = randperm(n) # une permutation aleatoire de (1..n)
sommeW = 0
for i = 1:n
...
# ------------------------------------------------------------
# archive des solutions (variables globales)
...
# ------------------------------------------------------------
# preconditionnement : scinde x en 2 vecteurs d'indices

split(s0)

# ------------------------------------------------------------
# procede a une exploration elementaire par swap aleatoires

#heuExplore(s0, zBest)
#@printf "\n\nzBest=%s \n" zBest

# ------------------------------------------------------------
# Recherche d'une bonne solution avec un recuit simule

# mouvement 1 pour flirter avec le glouton a sommeX variable
t0 = 300 ; lPalier = ceil(Int64, 1.5*n) ; α = 0.95 ; tLow =0.5
@printf "t0 = %d, lPalier = %d, α=%f \n" t0 lPalier α
sBest=metaSA(s0, t0, lPalier, α, addOrDrop, tLow, verbose)

# mouvement 2 pour creuser autour du glouton a sommeX fixe
t0 = 100 ; lPalier = ceil(Int64, 2.5*n) ; α = 0.8 ; tLow =0.05
@printf "t0 = %d, lPalier = %d, α=%f \n" t0 lPalier α
sBest=metaSA(sBest, t0, lPalier, α, swap, tLow, verbose)

# ------------------------------------------------------------
# sortie graphique de resultats

#Pkg.add("PyPlot") # necessaire avant le tout premier tracage
using PyPlot
traceSolutionsSA()
#traceTemperatureSA() #astuce : pas trouve comment activer 2 traces differents

# calcul et tracage de schema de refroidissement
# examenRefroidissement(t0=100 , α=0.7,  lgPalier=6 , tLow = 1)
# examenRefroidissement(100, 0.7, 6, 1)

# resume en chiffres des resultats
